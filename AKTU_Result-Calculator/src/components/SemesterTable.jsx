        ...
        <div className="card flex flex-col p-6 sm:p-8 rounded-2xl w-full max-w-2xl">
                ...
                <table className="w-full mb-6 text-sm sm:text-base" role="table" aria-label={`Semester ${semesterNumber} subjects`}>
                    ...
                    {subjects.map((subject, index) => (
                        <tr key={index} className="border-b border-gray-200 hover:bg-primary-50 transition-colors">
                                <td className="py-3 px-4 text-gray-800 font-medium">{subject} <span className="text-gray-500">({credits[index] || 0})</span></td>
                                    ...
                                    if (nextThreshold) {
                                        ...
                                        return (
                                            <div className="absolute top-1/2 right-4 transform -translate-y-1/2 z-10 group">
                                                <div role="img" aria-label={`+${diff} marks needed`} className="w-5 h-5 bg-accent-100 border border-accent-300 text-accent-700 rounded-full flex items-center justify-center text-xs font-bold cursor-help shadow-sm animate-pulse">
                                                    i
                                                </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <button
                    ...
                    aria-label={`Calculate SGPA for semester ${semesterNumber}`}
                >
                    Calculate SGPA
                </button>
